...
# 动态加载 ROS2 环境路径
def _load_ros2_environment():
    """在运行时动态加载 ROS2 环境，确保 rclpy 可用"""
    try:
        # 直接添加 ROS2 路径到 sys.path，优先级顺序很重要
        ros_paths = [
            os.path.expanduser("~/arm_ws/install/arm_msg/local/lib/python3.10/dist-packages"),
            os.path.expanduser("~/arm_ws/install/arm_msg/lib/python3.10/site-packages"),
            os.path.expanduser("~/ros2_ws/install/drone_interfaces/local/lib/python3.10/dist-packages"),
            os.path.expanduser("~/ros2_ws/install/drone_interfaces/lib/python3.10/site-packages"),
        ...
        try:
            import rclpy
            print(f"[Info] ROS2 环境已加载，rclpy 模块已导入")
        except ImportError as e:
            print(f"[Warning] 无法导入 rclpy: {e}")
    except Exception as e:
        print(f"[Warning] 自动加载 ROS2 环境失败: {e}")
