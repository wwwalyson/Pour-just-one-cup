    ...
    async def speech_to_text(
        ...
        while retry_count < MAX_RETRIES:
            try:
                ...
                text = lang_tag_filter(raw_text) if raw_text else ""
                display_text = text.get("content", "") if isinstance(text, dict) else text
                logger.bind(tag=TAG).debug(
                    f"语音识别耗时: {time.time() - start_time:.3f}s | 结果: {display_text}"
                )
